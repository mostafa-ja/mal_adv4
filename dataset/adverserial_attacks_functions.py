...
def PGD_Max(x,y, model, insertion_array, removal_array, attack_list = ['linf', 'l2', 'l1'],steps_max=5, is_sample = False, varepsilon = 1e-20):
    """
...
def pgd_step(x, y, model, insertion_array, removal_array, norm, k, step_length):
    """
...
def StepwiseMax(
  ...
  for i, mini_step in enumerate(mini_steps):
      ...
      for norm in attack_list:
          ...
          perturbation = pgd_step(adv_x[~done], label[~done], model, insertion_array, removal_array, norm, mini_step, step_length)
...
def pgd_one_step(x, y, model, insertion_array, removal_array, step_lengths):
    """
    Projected Gradient Descent (PGD) adversarial attack for stepwise.
    :param y: Ground truth labels
    :param x: Feature vector
    ...
    val, _ = torch.topk(torch.abs(gradients), 1)
    perturbation_l1 = (torch.abs(gradients) >= val.expand_as(gradients)).float() * torch.sign(gradients).float()
...
def StepwiseMax_onestep(
  ...
  for step in range(steps):
      ...
      pertbx = pgd_one_step(adv_x[~done], label[~done], model, insertion_array, removal_array, step_lengths)
...
def pgd_one_step2(x, y, model, insertion_array, removal_array, step_lengths,x_initial):
    """
    Projected Gradient Descent (PGD) adversarial attack for stepwise.
    :param y: Ground truth labels
    :param x: Feature vector
    ...
    val, _ = torch.topk(torch.abs(gradients), 1)
    perturbation_l1 = (torch.abs(gradients) >= val.expand_as(gradients)).float() * torch.sign(gradients).float() * (val > 1e-10)
    x_next_l1 = torch.clamp(x_next + perturbation_l1 * step_length, min=0., max=1.)
...
def StepwiseMax_onestep2(
  ...
  for step in range(steps):
      ...
      pertbx = pgd_one_step2(adv_x[~done], label[~done], model, insertion_array, removal_array, step_lengths,x[~done])

      with torch.no_grad():
          pertbx = torch.vstack(pertbx).to(torch.float32)
